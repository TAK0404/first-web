...
from foodapp.forms import CommentForm, FoodForm,CustForm,AdminForm,CartForm,OrderForm
    ...
    if request.method=="POST":
        ...
        for o in Order.objects.raw(sql1):
            if o.CustEmail==request.session['CustId']:
                od=str(o.OrderId)
                return render(request,'orderlist.html',{'success':'Order placed sucessfully!!!'+str(o.OrderId), 'orderlist': try_value})
    else:
        pass
...
def getorder(request):
    ...
    return render(request,'orderlist.html',{'orderlist': orders, 'comment_form': comment_form})
